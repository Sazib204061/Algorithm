...
lli f(lli ind, lli cnt)
{
    ...
    if(mark[ind][cnt])return dp[ind][cnt];

    lli sum = -inf;

    sum = max(sum, f(ind+1, cnt)); //not consider ar[ind]
    sum = max(sum, (ar[ind]*(cnt+1))+f(ind+1, cnt+1)); //consider ar[ind]
